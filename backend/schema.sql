...
DO $$
BEGIN
    -- Only attempt to alter if the users table already exists in the current schema
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='users' AND table_schema = current_schema()) THEN
        -- Add new columns if they don't exist
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                       WHERE table_name='users' AND column_name='pharmacy_id' AND table_schema = current_schema()) THEN
            ALTER TABLE users ADD COLUMN pharmacy_id INTEGER REFERENCES pharmacy(id) ON DELETE CASCADE;
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                       WHERE table_name='users' AND column_name='role' AND table_schema = current_schema()) THEN
            ALTER TABLE users ADD COLUMN role VARCHAR(20) DEFAULT 'EMPLOYEE' CHECK (role IN ('ADMIN', 'EMPLOYEE'));
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                       WHERE table_name='users' AND column_name='is_active' AND table_schema = current_schema()) THEN
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='sales' AND table_schema = current_schema()) THEN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                       WHERE table_name='sales' AND column_name='pharmacy_id' AND table_schema = current_schema()) THEN
            ALTER TABLE sales ADD COLUMN pharmacy_id INTEGER REFERENCES pharmacy(id) ON DELETE CASCADE;
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                       WHERE table_name='sales' AND column_name='generated_by_user_id' AND table_schema = current_schema()) THEN
