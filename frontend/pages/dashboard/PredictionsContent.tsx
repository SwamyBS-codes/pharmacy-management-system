...
export default function PredictionsContent() {
    ...
    const { data: medicines, isLoading: isLoadingMeds } = useQuery<MedicineListItem[]>({
    ...
    React.useEffect(() => {
        if (!selectedMedId && medicines && medicines.length > 0) {
            setSelectedMedId(String(medicines[0].id));
        }
    ...
    const { data: forecastData, isLoading: isLoadingForecast } = useQuery<ForecastData>({
        ...
        enabled: !!selectedMedId && !isLoadingMeds,
        staleTime: 1000 * 60 * 5,
    });
